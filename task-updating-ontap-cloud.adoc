...
BlueXP bietet drei Möglichkeiten für ein Cloud Volumes ONTAP Upgrade:
...
NOTE: Derzeit können Sie ein Upgrade auf die neueste verfügbare ONTAP-Version nur über die BlueXP Benutzeroberfläche durchführen. In einem zukünftigen Release wird es möglich sein, ein Upgrade auf eine bestimmte ONTAP Version über die BlueXP Benutzeroberfläche durchzuführen.
...
+
https://["NetApp KB: So konfigurieren Sie BlueXP als HTTPS-Server, um Upgrade-Images zu hosten"^]
...
https://["Erfahren Sie mehr über den Aufruf der /occm/config API"^].
