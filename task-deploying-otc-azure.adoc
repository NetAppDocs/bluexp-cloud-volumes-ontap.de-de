...
+
Ab Cloud Volumes ONTAP 9.15.1 können Sie VM-Instanzen in einzelnen Verfügbarkeitszonen des lokal redundanten Storage (LRS) bereitstellen. Dazu muss die `Microsoft.Compute/VMOrchestratorZonalMultiFD` Funktion auch für Ihr Abonnement aktiviert sein. Diese Funktion erleichtert die Implementierung von HA-Paaren (High Availability, Hochverfügbarkeit) in derselben Verfügbarkeitszone. Siehe link:task-saz-feature.html["Aktivieren Sie VMOrchestratorZonalMultiFD für einzelne Verfügbarkeitszonen"] und link:task-azure-high-availability-mode.html["Aktivieren Sie den Hochverfügbarkeits-Modus in Azure"].
...
+
*** Wählen Sie für einzelne Verfügbarkeitszonen eine Azure-Region, eine Verfügbarkeitszone, ein vnet und ein Subnetz aus. Ab Cloud Volumes ONTAP 9.15.1 können Sie VM-Instanzen in einer einzigen Verfügbarkeitszone installieren. Sie müssen eine Region auswählen, die diesen Modus unterstützt. Wenn die von Ihnen ausgewählte Region keine zonale Bereitstellung unterstützt, wird das vorherige nicht zonale Bereitstellungsmodell für LRS verwendet.
+
Weitere Informationen zu diesem Implementierungsmodus und den unterstützten Azure-Zonen finden Sie unter https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/["Microsoft Azure-Dokumentation: Virtual Machine Scale Sets Dokumentation"^] und https://azure.microsoft.com/en-us/explore/global-infrastructure/products-by-region/["Microsoft Azure-Website: Produkte nach Region verfügbar"^].
