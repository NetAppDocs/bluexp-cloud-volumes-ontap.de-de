...
* Bei der Konfiguration von Flash Cache für Cloud Volumes ONTAP 9.12.0 oder eine frühere Version auf AWS muss die Komprimierung auf allen Volumes deaktiviert werden, um die Performance-Verbesserungen des Flash Cache nutzen zu können. Wenn Sie Cloud Volumes ONTAP 9.12.1 oder höher bereitstellen oder ein Upgrade durchführen, muss die Komprimierung nicht deaktiviert werden.
+
