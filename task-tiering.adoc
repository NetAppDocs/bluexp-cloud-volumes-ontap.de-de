...
[role="quick-margin-para"]
Daten-Tiering sollte auf einem Aggregat aktiviert werden, um es auf einem Volume zu ermöglichen. Die Anforderungen für neue Volumes und vorhandene Volumes sollten Sie kennen. <<Ensuring that tiering is enabled on aggregates,Weitere Informationen .>>.
...
+
BlueXP  erstellt beim Erstellen des Systems einen Objektspeicher für kalte Daten<<Enabling data tiering after implementing the requirements,Solange es keine Verbindungs- oder Berechtigungsprobleme gibt>>. Danach müssen Sie nur Daten-Tiering auf Volumen aktivieren (und in einigen Fällen, <<Ensuring that tiering is enabled on aggregates,Auf Aggregaten>>).
