...
=== Anforderungen für das Tiering von Daten für einen Azure Mandanten

Sie können denselben Connector verwenden, um Daten für mehrere Azure Mandanten zu verschieben. Um in einem Szenario, in dem die Cloud Volumes ONTAP-Arbeitsumgebung von einem Mandanten erstellt und der Connector von einem anderen erstellt wird, Tiering für einen Azure Mandanten zu aktivieren, müssen Sie Folgendes sicherstellen:

* Sie haben die Anmeldeinformationen für beide Mandanten in BlueXP  zur Kreuzvalidierung eingegeben. Siehe https://docs.netapp.com/us-en/bluexp-setup-admin/task-adding-azure-accounts.html["Azure Zugangsdaten und Marketplace-Abonnements für BlueXP managen"^].
* Sie haben das virtuelle Netzwerk (vnet) Peering zwischen den beiden Mandanten aktiviert https://learn.microsoft.com/en-us/azure/virtual-network/virtual-network-peering-overview["Im Azure-Portal"^]. Dies ist erforderlich, damit der Connector auf das Speicherkonto des anderen Mandanten zugreifen kann.
* Sie müssen link:task-enabling-private-link.html#disable-azure-private-links-and-use-service-endpoints-instead["Private Links für Azure deaktiviert"]den Subnetz-Service-Endpunkt verwenden.
