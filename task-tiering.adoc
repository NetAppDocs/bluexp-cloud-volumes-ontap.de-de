...
Stellen Sie sicher, dass Cloud Volumes ONTAP eine Verbindung zu S3 hat. Die beste Möglichkeit, diese Verbindung bereitzustellen, besteht darin, einen VPC-Endpunkt für den S3-Dienst zu erstellen. Anweisungen hierzu finden Sie unter https://["AWS Dokumentation: Erstellen eines Gateway-Endpunkts"^].
...
----
Die Berechtigungen sind standardmäßig in die benutzerdefinierte Rolle enthalten. https://["Zeigen Sie die Azure-Berechtigung für den Connector an"^]
...
* Das Subnetz, in dem Cloud Volumes ONTAP residiert, muss für privaten Google-Zugriff konfiguriert werden. Anweisungen finden Sie unter https://["Google Cloud Documentation: Configuring Private Google Access"^].
...
+
Unterstützung bei der Datenreplizierung finden Sie unter https://["Replizierung von Daten in die und aus der Cloud"^].
