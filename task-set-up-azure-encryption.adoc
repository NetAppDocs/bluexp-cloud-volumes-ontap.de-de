...
[role="lead"]
Die Daten werden auf Cloud Volumes ONTAP in Azure automatisch verschlüsselt https://["Azure Storage Service Encryption"] Mit einem von Microsoft gemanagten Schlüssel Aber Sie können Ihren eigenen Verschlüsselungsschlüssel verwenden, indem Sie die Schritte auf dieser Seite befolgen.



== Übersicht über die Datenverschlüsselung

Cloud Volumes ONTAP-Daten werden in Azure automatisch verschlüsselt https://["Azure Storage Service Encryption"^]. Bei der Standardimplementierung wird ein von Microsoft verwalteter Schlüssel verwendet. Es ist keine Einrichtung erforderlich.
...
* Für Root-, Boot- und Datenfestplatten verwendet BlueXP einen https://["Festplattenverschlüsselungssatz"^], Die das Management von Schlüsseln mit verwalteten Festplatten ermöglicht.
* Neue Festplatten verwenden ebenfalls denselben Festplattenverschlüsselungssatz.
...
.Schritte
. https://["Erstellen Sie einen Schlüsselspeicher in Ihrem Azure-Abonnement"^].
+
...
. https://["Einen Schlüssel im Schlüsselspeicher erzeugen"^].
+
...
----
https://["Zeigen Sie die aktuelle Liste der Berechtigungen an"^]
...
+
https://["Weitere Informationen zu diesem API-Aufruf"^].
