...
Wenn Sie eine neue Version Ihres Schlüssels erstellen, verwendet Cloud Volumes ONTAP automatisch die neueste Schlüsselversion, um sicherzustellen, dass Ihre Daten sicher bleiben, ohne dass manuelle Updates erforderlich sind.

.Schritte
. Navigieren Sie im Azure-Portal zu Ihrem Schlüsselspeicher.
. Wählen Sie die Taste aus, die Sie drehen möchten.
. Klicken Sie auf *Neue Version*, um eine neue Version des Schlüssels zu generieren.


Sobald eine neue Version erstellt wurde, erkennt Cloud Volumes ONTAP automatisch die neueste Version des Schlüssels und verwendet diese für die Verschlüsselung.
