...
Sie sollten Ihre Verschlüsselungsschlüssel im Azure-Portal konfigurieren. Informationen zum Konfigurieren der Tasten und zum Einrichten der Tastenrotation finden Sie in den folgenden Microsoft Azure-Dokumentationsmaterialien:
...
NOTE: Nach der Konfiguration der Tasten, stellen Sie sicher, dass Sie haben https://learn.microsoft.com/en-us/azure/virtual-machines/disk-encryption#customer-managed-keys["Schlüsselrotation im Azure-Portal aktiviert"^], so dass Cloud Volumes ONTAP die neuen Tasten verwenden kann.

Beim Erstellen und Aktivieren einer neuen Version Ihrer Schlüssel erkennt und verwendet Cloud Volumes ONTAP automatisch die neueste Version für die Verschlüsselung. So wird sichergestellt, dass Ihre Daten sicher bleiben, ohne dass manuelle Updates erforderlich sind.
