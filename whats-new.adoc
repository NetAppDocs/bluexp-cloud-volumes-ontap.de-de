...
Die integrierten Datensicherungs- und Sicherheitsfunktionen von WORM (Write Once Read Many) und ARP (Autonomous Ransomware Protection) sind ohne zusätzliche Kosten in allen Cloud Volumes ONTAP Lizenzen enthalten. Dies gilt für neue und bestehende BYOL- und PAYGO/Marketplace-Implementierungen sowohl für Node-basierte als auch kapazitätsbasierte Lizenzen. Das neue Preismodell gilt für alle Cloud-Provider und Implementierungstypen, einschließlich Single-Node- und HA-Konfigurationen (Hochverfügbarkeit).

Die folgenden Vorteile werden verfügbar sein:

* Bestehende Konten mit WORM und ARP werden für diese Funktionen nicht mehr in Rechnung gestellt. Ihre Kapazitätsnutzung wird Ihnen entsprechend den vor dieser Änderung geltenden Anforderungen in Rechnung gestellt. WORM und ARP werden bei zukünftigen Abrechnungen ausgeschlossen.
* Wenn Sie diese Funktionen in Ihren bestehenden Konten nicht aktiviert haben, können Sie sich jetzt für WORM und ARP ohne zusätzliche Kosten entscheiden.
* Bei neuen Konten werden WORM- und ARP-Gebühren von allen Cloud Volumes ONTAP-Angeboten ausgeschlossen.
...
=== Unterstützte Mindestversion von ONTAP für Flash Cache auf Azure

Die ONTAP-Mindestversion, die für die Konfiguration von Flash Cache auf Azure erforderlich ist, ist 9.13.1 GA. ONTAP 9.13.1 GA und höhere Versionen können Sie für die Bereitstellung von Flash Cache auf Cloud Volumes ONTAP Systemen für Azure nur verwenden.
...
Informationen zum Konfigurieren und Verwenden von NDMP finden Sie unter https://docs.netapp.com/us-en/ontap/ndmp/index.html["NDMP-Konfiguration – Überblick"].
